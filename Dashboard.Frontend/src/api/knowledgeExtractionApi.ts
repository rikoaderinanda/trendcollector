...
export const knowledgeExtractionApi = {
  async getJobs(filters: KnowledgeExtractionJobsFilters = {}): Promise<KnowledgeExtractionJobDto[]> {
    const { data } = await knowledgeExtractionAxios.get<KnowledgeExtractionJobDto[]>(
  ...
  async getVideoDetail(videoId: number): Promise<KnowledgeExtractionDetailDto> {
    const { data } = await knowledgeExtractionAxios.get<KnowledgeExtractionDetailDto>(
      `/knowledge-extraction/video/${videoId}`,
    );
    return data;
  },

  async enqueueVideo(videoId: number, priority = 0): Promise<KnowledgeExtractionJobDto> {
    const { data } = await knowledgeExtractionAxios.post<KnowledgeExtractionJobDto>(
  ...
  async runExtraction(videoId: number): Promise<RunKnowledgeExtractionResponse> {
    const { data } = await knowledgeExtractionAxios.post<RunKnowledgeExtractionResponse>(
