...
public sealed class JobRepository : IJobRepository
{
    ...
    public async Task<long> CreateAsync(CollectionJob job, CancellationToken cancellationToken = default)
    {
        ...
        const string sql = """
            INSERT INTO collection_jobs (started_at, keyword, country, language, status)
            VALUES (@StartedAt, @Keyword, @Country, @Language, @Status)
            RETURNING id;
            """;

        return await connection.ExecuteScalarAsync<long>(sql,
            new { job.StartedAt, job.Keyword, job.Country, job.Language, job.Status }, commandTimeout: 30);
    }
    ...
    public async Task<IEnumerable<CollectionJob>> ListAsync(int limit, int offset, CancellationToken cancellationToken = default)
    {
        await using var connection = await _connectionFactory.CreateConnectionAsync(cancellationToken);
