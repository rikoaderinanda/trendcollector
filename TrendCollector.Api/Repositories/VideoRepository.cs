...
public sealed class VideoRepository : IVideoRepository
{
    ...
    public async Task<IEnumerable<TrendingVideo>> ListAsync(string? language, int limit, int offset, CancellationToken cancellationToken = default)
    {
        await using var connection = await _connectionFactory.CreateConnectionAsync(cancellationToken);
        ...
        return await connection.QueryAsync<TrendingVideo>(sql,
            new { Language = language, Limit = limit, Offset = offset }, commandTimeout: 30);
    }
}